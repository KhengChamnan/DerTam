...
class _DertamHotelScreenState extends State<DertamHotelScreen> {
  String selectedLocationName = 'All';
  int selectedLocationId = 0; // 0 means "All" provinces
  ...
  Future<void> _initializeScreen() async {
    ...
    try {
      ...
      final locations = await busBookingProvider.fetchListLocation();

      if (mounted) {
        setState(() {
          _locations = locations;
          // Keep default as "All" with ID 0
          selectedLocationName = 'All';
          selectedLocationId = 0;
        });

        // Fetch hotels for all provinces (province_id = 0)
              ...
              if (_locations.isEmpty)
                ...
                Flexible(
                  child: ListView.builder(
                    shrinkWrap: true,
                    itemCount: _locations.length + 1, // +1 for "All" option
                    itemBuilder: (context, index) {
                      // First item is "All"
                      if (index == 0) {
                        final isSelected = selectedLocationId == 0;
                        return ListTile(
                          title: Text(
                            'All',
                            style: TextStyle(
                      ...
                      // Other items are provinces
                      final location = _locations[index - 1];
                      final isSelected =
                          location.provinceCategoryID == selectedLocationId;
                ...
                child: Transform.translate(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Expanded(child: SingleChildScrollView(child: hotelList)),
                    ],
                  ),
                ),
              ),
