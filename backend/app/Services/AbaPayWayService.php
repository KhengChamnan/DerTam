...
class AbaPayWayService
{
    ...
    public function createPurchase(array $data)
    {
        ...
        if (empty($this->merchantId) || empty($this->apiKey) || empty($this->purchaseEndpoint)) {
            Log::error('ABA PayWay configuration is incomplete', [
                'merchant_id_set' => !empty($this->merchantId),
                'api_key_set' => !empty($this->apiKey),
                'purchase_endpoint_set' => !empty($this->purchaseEndpoint),
            ]);
            
            return [
                'success' => false,
                'error' => 'ABA PayWay is not properly configured',
            ];
        }
        ...
        // Log transaction ID validation
        Log::info('ABA PayWay - Transaction ID Validation', [
            'tran_id' => $tranId,
            'length' => strlen($tranId),
            'is_alphanumeric' => ctype_alnum($tranId),
            'amount' => $amount,
            'req_time' => $reqTime,
        ]);
        ...
        Log::info('ABA PayWay Purchase Request', [
            'tran_id' => $tranId,
            'tran_id_length' => strlen($tranId),
            'amount' => $amount,
            'payment_option' => $paymentOption,
            'post_data' => $postData, // Log full post data
        ]);
        ...
        try {
            ...
            // If not successful, extract error message from response
            if (!$isSuccessful) {
                $errorMessage = 'ABA PayWay request failed';
